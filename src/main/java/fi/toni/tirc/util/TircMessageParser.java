...
public class TircMessageParser {
	...
	public static TircLine parseArrived(List<DBObject> locations) {

		List<String> locationsStr = locations
				.stream()
				.map(dbobject -> {
					BasicDBObject basicDbObject = (BasicDBObject) dbobject;
					BasicDBObject location = (BasicDBObject) basicDbObject
							.get("location");
					String latitude = location.getString("latitude");
					String longitude = location.getString("longitude");
					return latitude + "," + longitude;
				}).collect(Collectors.toList());
		BasicDBObject lastObject = (BasicDBObject) locations.get(locations
				.size() - 1);
