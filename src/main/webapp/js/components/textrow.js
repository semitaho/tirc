   ...
   isJoinOrPart : function(input) {
    return input.indexOf("has joined #") !== -1
	  ...
	  rendercomment: function(item){
		  ...
		  return (<div className={textrowstyle}>
		  	...
		  	<div className="nicktext">
		  		<div className="text" dangerouslySetInnerHTML={{__html: item.line + '<span class="time"> - '+item.time+'</span>'}}>
		  			</div>
	  ...
	  renderme: function(item){
		  return (<div className="textrow">
					<div className="action" dangerouslySetInnerHTML={{__html: item.line + '<span class="time"> - '+item.time+'</span>'}}></div>
				 </div>)
	  },
    ...
    renderwelcome: function(item){
      return (<div className="textrow">
          <div className="action" dangerouslySetInnerHTML={{__html: item.nick +' saapui paikalle ' +item.line + '<span class="time"> - '+item.time+'</span>'}}></div>
         </div>)
    },

    renderaction : function(item){
      return (<div className="textrow">
          <div className="star" dangerouslySetInnerHTML={{__html: item.line + '<span class="time"> - '+item.time+'</span>'}}></div>
         </div>)
    },

	  renderquit: function(item){
		  return (<div className="textrow">
		  				<div className="quit">{item.nick} poistui kokonaan ircistä: {item.line} - {item.time}</div>
		  		
		  		 </div>)
	  },
	  renderjoin: function(item){
			  return (<div className="textrow">
			  				<div className="quit">{item.nick} saapui kanavalle - {item.time}</div>
	  ...
	  render : function(){
		...
		return <div className="action">{this.props.elem} <span className="time"> - {this.props.time}</span></div>
	  },

    renderlogevent : function(item){
      return <div className="textrow logevent">{item.line}</div>;
    }     
